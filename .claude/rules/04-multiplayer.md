...
모든 플레이어는 세션 시작 시 **한 번에 같이 들어온다.** 진행 중인 게임에 나중에 합류하는(join-in-progress) 클라이언트는 없다.

따라서 늦게 들어온 클라이언트에게 현재 상태를 따라잡게 하는 재동기화 경로는 만들지 않는다. 이미 진행된 이벤트를 뒤늦게 재생하거나, 접속 시점에 월드 상태를 통째로 복구하는 로직은 이 프로젝트에 필요 없다.

단, **각 클라이언트의 로딩·초기화 완료 시점은 여전히 서로 다르다.** "다 같이 시작한다"가 "모든 액터의 `BeginPlay`가 같은 프레임에 돈다"는 뜻은 아니다. 초기화 순서 의존은 여전히 버그가 된다.
